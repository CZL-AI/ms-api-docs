...
## 调用接口：
**请求方式：** `POST（HTTPS）`
**请求地址：** `https://ms-ai.chongzhiling.com/api/v1.0/ai-b/aipic-exotic/report?token=[ACCESS_TOKEN]`
...
| img_url         | string | 是   | 识别异宠图片地址（需提供一张能正常访问的图片地址） |
| pet_profile_id  | number | 是   | 宠物档案ID                                         |
| sub_module_type | number | 是   | 子模块类型，固定为1（异宠品种识别）                |
| report_url      | string | 否   | 报告URL（可选参数，默认为空字符串）                |
