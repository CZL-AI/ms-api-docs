...
## 接口描述
---
该接口用于获取单条医疗记录的完整报告内容。默认情况下返回 JSON 格式的详细报告数据，用户也可以通过参数指定返回 HTML 格式的报告内容或报告链接。

该接口是**通用报告详情接口**，并非只适用于智能问诊。不同 `module_type` 下，`report` 字段结构会有所差异。
...
| report_id | number | 是   | 报告 ID，通常由列表接口返回，例如 `/medical-record-list`、`/pet-profile/advice-records` 返回的 `id` 或 `medical_record_id` |
...
> 以下示例以智能问诊报告为例。若 `module_type=19`（宠物档案建议），建议优先使用专用详情接口 [`/pet-profile/advice-record`](../pet-profile-advice/advice-record.md)。
...
| sub_module_type   | string | 子模块类型，不同 `module_type` 下含义不同。例如 `module_type=19` 时，`1`-喂养建议，`2`-养成建议 |
...
## 注意事项
1. **默认行为**：接口默认返回详细 JSON 数据，保持与原有接口的兼容性。
2. **格式选择**：通过 `format` 和 `html_type` 参数组合可以灵活选择返回格式。
3. **结构差异**：`report` 字段会随 `module_type` 变化，不同业务模块的 JSON 结构并不完全一致。
4. **专用接口优先**：宠物档案建议模块建议优先使用 [`/pet-profile/advice-record`](../pet-profile-advice/advice-record.md) 查看详情。
5. **安全性**：通过链接方式访问报告时，链接中包含访问令牌以确保安全性。
